<template>
    <div class="modal fade" id="kt_modal_add_supplier_for_purchase" ref="addSupplierModalRef" tabindex="-1" aria-hidden="true">
        <!--begin::Modal dialog-->
        <div class="modal-dialog modal-dialog-centered mw-650px">
            <div class="modal-content">
                ...
                <div class="modal-header" id="kt_modal_update_email_header">
                    <!--begin::Modal title-->
                    <h2 class="fw-bold">{{ $t('supplier.title.add') }}</h2>
                    <!--end::Modal title-->
...
const addSupplierModalRef = ref < null | HTMLElement > (null);
...
const submit = () => {
  const url = props.routeType === 'other'
    ? route('purchases.others.supplier.store')
    : route('purchases.supplier.store');

  formData.post(url, {
    preserveScroll: true,
    onSuccess: () => {
      hideModal(addSupplierModalRef.value);
