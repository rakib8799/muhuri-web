            ...
            <div class="card-header border-0 cursor-pointer">
                <div class="card-title m-0">
                    <h3 class="fw-bold m-0">
                        {{ props?.supplier?.id ? $t("supplier.title.edit") : $t("supplier.title.add") }}
                    </h3>
                </div>
            </div>
            <!--end::Card header-->
                ...
                <VForm @submit="submit()" class="form">
                    ...
                    <div class="card-body border-top p-9">
                        ...
                        <div class="row mb-5 g-4">
                            <!-- Previous Due -->
                            <div class="col-md-6 fv-row">
                                <label class="fs-5 fw-semibold mb-2">{{ $t("supplier.label.previousDue") }}</label>
                                <Field type="number" class="form-control form-control-lg form-control-solid" :placeholder="$t('supplier.placeholder.previousDue')" name="previous_due" v-model="formData.previous_due" />
                    ...
                    <div class="card-footer d-flex justify-content-end py-6 px-9">
                        <SubmitButton :id="props.supplier?.id" />
                    </div>
                </VForm>
            </div>
        </div>
...
const formData = useForm({
    ...
    previous_due: props.supplier?.previous_due || 0,
    division_id: props.supplier?.division_id || "",
    district_id: props.supplier?.district_id || "",
    upazila_id: props.supplier?.upazila_id || "",
    union_id: props.supplier?.union_id || "",
    village: props.supplier?.village || "",
...
const submit = () => {
    if (props.supplier?.id) {
        ...
        formData.patch(route("suppliers.update", props.supplier?.id), {
            preserveScroll: true,
        });
    } else {
        // for adding new supplier
        formData.post(route("suppliers.store"), {
            preserveScroll: true,
        });
    }
};
