        ...
        <div class="card mb-5 mb-xl-10">
            ...
            <div class="card-header border-0 cursor-pointer">
                <div class="card-title m-0">
                    <h3 class="fw-bold m-0">{{ $t('label.editItem') }}</h3>
                </div>
            </div>
            <!--end::Card header-->
            ...
            <div class="show">
                ...
                <div style="margin-left: 32px; margin-top: -63px; margin-bottom: 20px">
                    <DeleteConfirmationButton
                        v-if="checkPermission('can-delete-sale')"
                        :obj="sale"
                        confirmRoute="sales.destroy"
                        :buttonText="$t('sale.buttonValue.delete')"
                        :showIcon="false"
                        :messageTitle="`${$t('confirmation.sale.delete', { buyer: props?.buyer?.name, saleDate: props?.sale?.sale_date})}`"
                    />
                </div>
            </div>
        </div>
        <UpdateItemModal :item="editItem" :items="allItems" :itemUnits="props?.itemUnits" :brands="allBrands" :sale="props?.sale" @updateSaleItem="updatedSaleItem"></UpdateItemModal>
    </AuthenticatedLayout>
</template>
...
import UpdateItemModal from "@/Pages/Sale/Modal/UpdateItemModal.vue";
...
onMounted(() => {
    if (props.sale) {
        ...
        tableData.value = props.sale?.sale_items.map((sale: any) => {
            ...
            const quantity = sale.total_poly * sale.mir;
            const total_price = Math.round((quantity * sale.unit_price) / itemUnitValue);
            return {
                id: sale.id,
                item_id: itemName?.id,
                ...
                total_box: (itemUnit.name === 'million') ? 0 : sale?.total_box || 0,
                total_poly: (itemUnit.name === 'million') ? 0 : sale.total_poly,
                mir: (itemUnit.name === 'million') ? 0 : sale.mir,
                unit_price: sale.unit_price,
                quantity: (itemUnit.name === 'million') ? sale.quantity : quantity,
                total_price: (itemUnit.name === 'million') ? sale.total_price : total_price,
                note: sale.note
            };
        });
    }
...
const submit = () => {
    if((validatedData())){
        formData.patch(route('sales.update', props.sale?.id),{
            onSuccess: () => {
                preserveScroll: true
            },
            onError: (errors) => {
                console.log("Sale Update Error:",errors);
            }
        });
    }
