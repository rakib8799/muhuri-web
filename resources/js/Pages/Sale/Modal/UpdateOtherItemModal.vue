<template>
    <div class="modal fade" id="kt_modal_update_other_item" ref="updateOtherSaleModalRef" tabindex="-1" aria-hidden="true">
...
const updateOtherSaleModalRef = ref<null | HTMLElement>(null);
...
const emit = defineEmits(['updateOtherSaleItem']);

const submit = async () => {
    if(validateForm()){
        try {
            const url = route('sales.others.saleItems.update', {
                sale: props.sale?.id,
            ...
            emit('updateOtherSaleItem', response.data);

            hideModal(updateOtherSaleModalRef.value);
        } catch (error) {
