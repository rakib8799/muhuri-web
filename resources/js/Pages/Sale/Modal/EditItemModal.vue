<template>
    <div class="modal fade" id="kt_modal_edit_sale" ref="editSaleModalRef" tabindex="-1" aria-hidden="true">
...
const validateForm = () => {
    ...
    if(!hasUnitRelation.value){
        ...
        if (formData.total_poly < 0) {
            errors.total_poly = "Total Poly must be a non negative number";
            valid = false;
        }
        ...
        if (formData.mir < 0) {
            errors.mir = "Mir must be a non negative number";
            valid = false;
        }
    }
...
const submit = () => {
    if (validateForm()) {
        ...
        formData.brand_id = props.item?.brand_id;
        formData.brand_name = props.item?.brand_name;
        formData.total_box = hasUnitRelation.value ? 0 : props.item?.total_box;
        formData.total_poly = hasUnitRelation.value ? 0 : props.item?.total_poly;
        formData.mir = hasUnitRelation.value ? 0 : props.item?.mir;
        formData.quantity = hasUnitRelation.value ? 0 : props.item?.quantity;
        formData.unit_price = props.item?.unit_price;
