<template>
    <div class="modal fade" id="kt_modal_add_sale" ref="addSaleModalRef" tabindex="-1" aria-hidden="true">
                        ...
                        <div class="scroll-y me-n7 pe-7">
                            ...
                            <div class="row mb-5 g-4">
                                ...
                                <div :class="!hasUnitRelation ? 'fv-row mb-7 col-md-6' : 'fv-row mb-7 col-md-12'">
                                    ...
                                    <Field type="number" :placeholder="$t('sale.placeholder.quantity')" :class="['form-control form-control-lg form-control-solid ', {'text-muted' : !hasUnitRelation}]" name="quantity" v-model="formData.quantity" :disabled="!hasUnitRelation"/>
...
const addSaleModalRef = ref<null | HTMLElement>(null);
...
const validateForm = () => {
    ...
    if(!hasUnitRelation.value){
        if (!formData.total_poly || isNaN(Number(formData.total_poly))) {
            errors.total_poly = "Total Poly must be a number";
            valid = false;
        }
        if (formData.total_poly <= 0) {
            errors.total_poly = "Total Poly must be a greater than 0";
            valid = false;
        }
        if (!formData.mir || isNaN(Number(formData.mir))) {
            errors.mir = "Mir must be a number";
            valid = false;
        }
        if (formData.mir <= 0) {
            errors.mir = "Mir must be a greater than 0";
            valid = false;
        }
    }
    ...
    if (formData.quantity <= 0) {
        errors.quantity = "Quantity must be a greater than 0";
        valid = false;
    }
    ...
    if (formData.unit_price <= 0) {
        errors.unit_price = "Unit Price must be a greater than 0";
        valid = false;
    }
    if (!formData.total_price || isNaN(Number(formData.total_price))) {
        errors.total_price = "Total Price Price must be a number";
        valid = false;
    }
    if (formData.total_price <= 0) {
        errors.total_price = "Total Price must be a greater than 0";
        valid = false;
    }
...
const submit = () => {
    console.log("hasUnitRelation", hasUnitRelation.value);
    if (validateForm()) {
        if(hasUnitRelation.value){
        ...
        hideModal(addSaleModalRef.value);
    }
};
</script>
