<template>
    <div class="modal fade" id="kt_modal_edit_other_purchase" ref="editOtherPurchaseModalRef" tabindex="-1" aria-hidden="true">
...
const editOtherPurchaseModalRef = ref<null | HTMLElement>(null);
...
const submit = () => {
    if (validateForm()) {
        ...
        formData.unit_price = !isTargetItem.value ? 0 : props.item?.unit_price;
        formData.total_price = props.item?.total_price;
        formData.note = props.item?.note;

        hideModal(editOtherPurchaseModalRef.value);
    }
};
</script>
