            ...
            <div class="show">
                                        ...
                                        <tr v-for="(purchase, index) in tableData" :key="purchase.id">
                                            ...
                                            <td class="text-end">
                                                <div class="d-flex align-items-center justify-content-end">
                                                    <ConfirmationButton
                                                        v-if="checkPermission('can-edit-purchase')"
                                                        iconName="pencil"
                                                        :obj="purchase"
                                                        title="Item Edit"
                ...
                <div style="margin-left: 32px; margin-top: -63px; margin-bottom: 20px">
                    <DeleteConfirmationButton
                        v-if="checkPermission('can-delete-purchase')"
                        :obj="purchase"
                        confirmRoute="purchases.destroy"
                        :buttonText="$t('purchase.buttonValue.delete')"
                        :showIcon="false"
                        :messageTitle="`${$t('confirmation.purchase.delete', { supplier: props?.supplier?.name, purchaseDate: props?.purchase?.purchase_date})}`"
                    />
                </div>
            </div>
        </div>
        <UpdateItemModal :item="editItem" :items="allItems" :itemUnits="props?.itemUnits" :brands="allBrands" :purchase="props?.purchase" @updatePurchaseItem="updatedPurchaseItem"></UpdateItemModal>
    </AuthenticatedLayout>
</template>
...
import UpdateItemModal from "@/Pages/Purchase/Modal/UpdateItemModal.vue";
...
const formData = useForm({
    ...
    purchase_note: props.purchase?.note || ''
});
...
onMounted(() => {
    if (props.purchase) {
        ...
        tableData.value = props.purchase?.purchase_items.map((purchase: any) => {
            ...
            const quantity = purchase.total_poly * purchase.mir;
            const total_price = Math.round((quantity * purchase.unit_price) / itemUnitValue);
            return {
                id: purchase.id,
                item_id: itemName?.id,
                ...
                total_box: purchase?.total_box || 0,
                total_poly: purchase.total_poly || 0,
                mir: purchase.mir || 0,
...
const updatedPurchaseItem = async () => {
    try {
...
const submit = () => {
    if((validatedData())){
        formData.patch(route('purchases.update', props.purchase?.id),{
            onSuccess: () => {
                preserveScroll: true
            },
            onError: (errors) => {
                console.log("Purchase Update Error:",errors);
            }
        });
    }
